# Parity Status — CYGNIS CODE Rust Port
...
- Mock parity harness stats: **10 scripted scenarios**, **19 captured `/v1/messages` requests** in `rust/crates/rusty-cygnis-cli/tests/mock_parity_harness.rs`.

## Mock parity harness — milestone 1

- [x] Deterministic CYGNIS-compatible mock service (`rust/crates/mock-cygnis-service`)
- [x] Reproducible clean-environment CLI harness (`rust/crates/rusty-cygnis-cli/tests/mock_parity_harness.rs`)
...
- **Why it matters:** `.github/workflows/rust-ci.yml` runs `cargo fmt --all --check` and `cargo test -p rusty-cygnis-cli`; this lane removed a CI-specific sandbox assumption from runtime behavior.
...
- [x] Config merge precedence (user > project > local) — `ConfigLoader::discover()` loads user → project → local, and `loads_and_merges_cygnis_code_config_files_by_precedence()` verifies the merge order.
