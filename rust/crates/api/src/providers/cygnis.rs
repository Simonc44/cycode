...
use telemetry::{AnalyticsEvent, CygnisRequestProfile, ClientIdentity, SessionTracer};
...
impl AuthSource {
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_env_non_empty("CYGNIS_API_KEY")?;
        let auth_token = read_env_non_empty("CYGNIS_AUTH_TOKEN")?;
        match (api_key, auth_token) {
            ...
            (None, None) => Err(ApiError::missing_credentials(
                "Anthropic",
                &["CYGNIS_AUTH_TOKEN", "CYGNIS_API_KEY"],
            )),
        }
    }
...
pub struct CygnisClient {
    ...
    request_profile: CygnisRequestProfile,
...
impl CygnisClient {
    ...
    pub fn new(api_key: impl Into<String>) -> Self {
        Self {
            ...
            request_profile: CygnisRequestProfile::default(),
            session_tracer: None,
            prompt_cache: None,
    ...
    #[must_use]
    pub fn request_profile(&self) -> &CygnisRequestProfile {
    ...
    #[must_use]
    pub fn with_request_profile(mut self, request_profile: CygnisRequestProfile) -> Self {
...
impl AuthSource {
    pub fn from_env_or_saved() -> Result<Self, ApiError> {
        if let Some(api_key) = read_env_non_empty("CYGNIS_API_KEY")? {
            return match read_env_non_empty("CYGNIS_AUTH_TOKEN")? {
        ...
        if let Some(bearer_token) = read_env_non_empty("CYGNIS_AUTH_TOKEN")? {
...
pub fn has_auth_from_env_or_saved() -> Result<bool, ApiError> {
    Ok(read_env_non_empty("CYGNIS_API_KEY")?.is_some()
        || read_env_non_empty("CYGNIS_AUTH_TOKEN")?.is_some()
...
fn resolve_saved_oauth_token_set(
    ...
    let client = CygnisClient::from_auth(AuthSource::None).with_base_url(read_base_url());
...
fn read_auth_token() -> Option<String> {
    read_env_non_empty("CYGNIS_AUTH_TOKEN")
        .ok()
...
pub fn read_base_url() -> String {
    std::env::var("CYGNIS_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.to_string())
}
...
impl Provider for CygnisClient {
...
mod tests {
    ...
    use super::{
        ...
        resolve_startup_auth_source, CygnisClient, AuthSource, OAuthTokenSet,
    };
    ...
    fn read_api_key_requires_presence() {
        let _guard = env_lock();
        std::env::remove_var("CYGNIS_AUTH_TOKEN");
        std::env::remove_var("CYGNIS_API_KEY");
        std::env::remove_var("CYGNIS_CONFIG_HOME");
    ...
    fn read_api_key_requires_non_empty_value() {
        let _guard = env_lock();
        std::env::set_var("CYGNIS_AUTH_TOKEN", "");
        std::env::remove_var("CYGNIS_API_KEY");
    ...
    fn read_api_key_prefers_api_key_env() {
        let _guard = env_lock();
        std::env::set_var("CYGNIS_AUTH_TOKEN", "auth-token");
        std::env::set_var("CYGNIS_API_KEY", "legacy-key");
        assert_eq!(
    ...
    fn auth_source_from_saved_oauth_when_env_absent() {
        ...
        std::env::set_var("CYGNIS_CONFIG_HOME", &config_home);
        std::env::remove_var("CYGNIS_AUTH_TOKEN");
        std::env::remove_var("CYGNIS_API_KEY");
        save_oauth_credentials(&runtime::OAuthTokenSet {
    ...
    fn backoff_doubles_until_maximum() {
        let client = CygnisClient::new("test-key").with_retry_policy(
            3,
