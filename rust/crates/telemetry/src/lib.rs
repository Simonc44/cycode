...
pub const DEFAULT_CYGNIS_VERSION: &str = "2023-06-01";
pub const DEFAULT_APP_NAME: &str = "cygnis-code";
pub const DEFAULT_RUNTIME: &str = "rust";
pub const DEFAULT_AGENTIC_BETA: &str = "cygnis-code-20250219";
...
pub struct CygnisRequestProfile {
    pub cygnis_version: String,
...
impl CygnisRequestProfile {
    #[must_use]
    pub fn new(client_identity: ClientIdentity) -> Self {
        Self {
            cygnis_version: DEFAULT_CYGNIS_VERSION.to_string(),
            client_identity,
            betas: vec![
    ...
    pub fn header_pairs(&self) -> Vec<(String, String)> {
        let mut headers = vec![
            (
                "cygnis-version".to_string(),
                self.cygnis_version.clone(),
            ),
            ("user-agent".to_string(), self.client_identity.user_agent()),
        ];
        if !self.betas.is_empty() {
            headers.push(("cygnis-beta".to_string(), self.betas.join(",")));
        }
        headers
    }
...
impl Default for CygnisRequestProfile {
    fn default() -> Self {
...
mod tests {
    ...
    fn request_profile_emits_headers_and_merges_body() {
        let profile = CygnisRequestProfile::new(
            ClientIdentity::new("cygnis-code", "1.2.3").with_runtime("rust-cli"),
        )
        ...
        assert_eq!(
            ...
            vec![
                (
                    "cygnis-version".to_string(),
                    DEFAULT_CYGNIS_VERSION.to_string()
                ),
                ("user-agent".to_string(), "cygnis-code/1.2.3".to_string()),
                (
                    "cygnis-beta".to_string(),
                    "cygnis-code-20250219,prompt-caching-scope-2026-01-05,tools-2026-04-01"
                        .to_string(),
                ),
            ]
        );
        ...
        let body = profile
            .render_json_body(&serde_json::json!({"model": "cygnis-sonnet"}))
    ...
    fn session_tracer_records_structured_events_and_trace_sequence() {
        ...
        tracer.record_analytics(
            AnalyticsEvent::new("cli", "prompt_sent")
                .with_property("model", Value::String("cygnis-opus".to_string())),
        );
